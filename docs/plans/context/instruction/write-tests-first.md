...
このフェーズでは、未実装モジュールを解決するための空実装・スタブ・TODO をプロダクションコードへ追加してはいけません。未実装 import による失敗は、実装フェーズへ引き継ぐ期待失敗として記録します。
...
**必須出力:**
`../output_contracts/test-report.md` に従って、作成テストと実行結果を報告してください。想定内の失敗と、実装前でも修正が必要なエラーを分けて記録してください。
